...
It is not obvious from the above code but basically the above works if I run `session$setInputs(do_reducing_option1 = 1)` and `session$elapse(millis = 10000)` concurrently in the cosole (ie. if I highlight both and press enter or type both into the console and type enter) but it does not work if I run them one after the other. This indicates that there is some timing issue related to the `invalidateLater` function in `backgroundReduce`. The `invalidateLater` timing had previously been 250ms but which I upped to 2500ms to see if it solved the issue - it did not. I'm going to take
